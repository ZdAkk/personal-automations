...
// The SDK defaults to a 600s (10 minute) timeout with 2 retries, so a single
// stalled upstream call can block for ~30 minutes. That is invisible in a cron
// task (the caller just waits) and fatal in a request-scoped handler, so every
// call gets a real ceiling. 120s suits the long-running research models; short
// calls override it (see chat's timeoutMs).
const DEFAULT_TIMEOUT_MS = Number(process.env.OPENROUTER_TIMEOUT_MS ?? 120_000);

function getClient(timeoutMs = DEFAULT_TIMEOUT_MS): OpenAI {
  ...
  return new OpenAI({
    ...
    timeout: timeoutMs,
    // One retry, not two: a stalled call should surface quickly rather than
    // multiplying the worst case by three.
    maxRetries: 1,
    defaultHeaders: {
 ...
 * and `opts.timeoutMs` to tighten the ceiling for short prompts.
 */
export async function chat(
  ...
  const client = getClient(opts.timeoutMs);
