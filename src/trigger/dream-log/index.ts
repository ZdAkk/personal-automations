 ...
 *     → picks up pending entries from the active note provider
 ...
 *     → posts summary comment via provider, moves entry to "Done"
 *     → on error: moves to "Error", posts error comment
 *
 * Active provider: NOTE_PROVIDER env var (clickup | notion), default clickup.
 * Source collection: "dreams" (resolved internally by getNoteProvider).
...
import { getNoteProvider, NoteCollection } from "../../lib/providers/note-provider";
...
export const dreamLog = task({
  ...
  run: async (payload: {
    ...
    try {
      ...
      // Step 5 — Post summary comment via provider and mark done
      ...
      await provider.addComment(payload.entryId, comment);
      await provider.updateStatus(payload.entryId, "Done");

      logger.log("Dream log complete", { entryId: payload.entryId, dream_id });
      return { entryId: payload.entryId, dream_id };
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      logger.log("Dream log failed", { entryId: payload.entryId, error: message });
      await provider.updateStatus(payload.entryId, "Error").catch(() => {});
      await provider
        .addComment(
          payload.entryId,
...
// ---------------------------------------------------------------------------
// Poller — runs every 2 minutes, picks up pending entries from active provider
// ---------------------------------------------------------------------------
...
export const dreamLogPoller = schedules.task({
  ...
  run: async () => {
    ...
    logger.log("Polling for pending dream entries…");
    const entries = await provider.listByStatus("Raw");
    logger.log(`Found ${entries.length} pending dream(s)`);

    for (const entry of entries) {
      if (!entry.content) {
        logger.log("Skipping entry with empty content", { id: entry.id });
        continue;
      }

      await provider.updateStatus(entry.id, "Processing");

      await dreamLog.trigger(
        {
          entryId: entry.id,
          rawText: entry.content,
          dreamedOn: entry.createdAt,
        },
        { idempotencyKey: `dream-log-${entry.id}` }
      );

      logger.log("Triggered dream log", { id: entry.id, dreamedOn: entry.createdAt });
    }
  },
});
